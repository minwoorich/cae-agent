...
입력 파일 라이브러리에서는 여러 파일을 끌어놓고 파일명, 형식, 크기와 저장
시각을 확인할 수 있다. 채팅 첨부 체크박스는 현재 UI 세션의 선택 상태이며,
채팅 기능이 연결되기 전에는 Codex 요청이나 실행 승인으로 취급하지 않는다.

`대화` 화면에 `MOCK MODE`가 표시되면 메시지 전송, 스트리밍, 중지와 다시
시도 UI만 검증한다. 이 모드에서는 Codex CLI와 Ansys를 실행하지 않으며
표시된 답변을 실제 CAE 결과로 해석하지 않는다.
