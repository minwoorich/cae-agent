...
`Codex 읽기 전용`은 Codex의 명령·파일 변경 권한이며 Workbench 연결 여부와
관계없습니다. 로컬 브라우저를 구분하는 임의 세션 ID는 화면에 표시하지 않습니다.
...
시작됩니다. 다음 단계의 승인 카드가 완성되기 전에는 명령 실행과 파일 변경
승인 요청을 자동 거절합니다. 따라서 현재 화면에서는 저장소와 첨부 파일을
검토하고 계획이나 스크립트 초안을 대화로 받을 수 있지만 Workbench,
SpaceClaim, Mechanical 실행이나 파일 수정은 할 수 없습니다.
...
채팅 작성기의 `파일 첨부 또는 드래그앤드롭` 영역에 파일을 올리면
